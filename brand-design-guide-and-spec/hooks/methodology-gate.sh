...
#
# Kill switch: export BRAND_DESIGN_GUIDE_AND_SPEC_GATE_OFF=1
set -uo pipefail

role="${CLAUDE_ROLE:-brand-design}"
deny() { echo "${role}: refused — $1" >&2; exit 2; }

case "${BRAND_DESIGN_GUIDE_AND_SPEC_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
try:
    ...
    HEX_RE = re.compile(r'#[0-9a-f]{3,8}\b|rgba?\s*\(')

    missing = []

    # 1. Brand guide entry present — at least one concrete sub-field, or an
    #    explicit "not applicable: <reason>" early exit per sub-field.
    guide_present = has_any(
        "logo usage", "typography", "voice/tone", "voice와 tone", "imagery style",
        "do's/don't", "do's and don't", "do/don't",
    ) or bool(HEX_RE.search(low))
    not_applicable = "not applicable" in low or "해당 없음" in low
    if not (guide_present or not_applicable):
        missing.append("brand-guide-entry")

    # 2. Asset spec present — a second, distinct occurrence of a literal
    #    applied value (color/hex or file-format/path token), not just the
    #    brand guide entry's rule statement.
    hex_count = len(HEX_RE.findall(low))
    path_or_format = bool(re.search(r'\.(svg|png|jpg|jpeg|webp|otf|ttf|woff2?)\b', low)) or bool(re.search(r'\S+/\S+', new_text))
    asset_spec_present = hex_count >= 2 or (hex_count >= 1 and path_or_format) or (not_applicable and path_or_format)
    if not (asset_spec_present or (not_applicable and not guide_present)):
    ...
    if missing:
        deny(
            ...
            "entry's rule statement)." % ", ".join(missing)
        )
