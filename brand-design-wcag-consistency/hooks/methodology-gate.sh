...
#
# Kill switch: export BRAND_DESIGN_WCAG_CONSISTENCY_GATE_OFF=1
set -uo pipefail

role="${CLAUDE_ROLE:-brand-design}"
deny() { echo "${role}: refused — $1" >&2; exit 2; }

case "${BRAND_DESIGN_WCAG_CONSISTENCY_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
try:
    ...
    if new_text is None:
        deny(
            ...
            "Edit/MultiEdit whose old_string matches, so the consistency-check/WCAG fields "
    ...
    # Any color pairing being introduced at all — a text/background
    # combination mentioned in the record, unless the record explicitly
    # early-exits ("no new ... pairing introduced").
    early_exit = has_any("no new text/background pairing", "no new pairing", "no color pairing introduced")
    pairing_present = (not early_exit) and has_any("text/background", "contrast", "pairing")

    missing = []

    if pairing_present:
        ratio_present = bool(re.search(r'\d+(\.\d+)?\s*:\s*1', new_text))
        labeled = has_any("pass", "fail", " aa", "aa)", "aaa")
        if not (ratio_present and labeled):
            missing.append("wcag-contrast-ratio-and-verdict")
        if not has_any("pass", "fail"):
            missing.append("consistency-check-verdict")
    else:
        if not has_any("no new text/background pairing", "no new pairing", "no color pairing introduced"):
    ...
    if missing:
        deny(
            ...
            "4.5:1 (normal) or 3:1 (large); a record with no new pairing must say so "
            "explicitly (e.g. 'no new text/background pairing introduced')." % ", ".join(missing)
        )
